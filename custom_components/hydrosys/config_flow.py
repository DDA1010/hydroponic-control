"""Config and options flow for HydroSYS."""
...
def _user_schema() -> vol.Schema:
    """Schema for the initial wiring of existing entities.

    Optional entity selectors carry no default so that leaving one blank simply
    omits the key (feeding ``None`` into an EntitySelector would fail validation).
    """
    return vol.Schema(
        {
...
class HydroSysConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle the UI setup."""
    ...
    def async_get_options_flow(config_entry: ConfigEntry) -> OptionsFlow:
        ...
        return HydroSysOptionsFlow()


class HydroSysOptionsFlow(OptionsFlow):
