"""Core orchestration logic for Hydroponic Control.
...
class HydroController(DataUpdateCoordinator[HydroControlData]):
    ...
    def _fire_alert(self, alarm: str, severity: str, message: str) -> None:
        _LOGGER.debug("Hydroponic Control alert [%s/%s]: %s", alarm, severity, message)
        self.hass.bus.async_fire(
            EVENT_ALERT,
            {
        ...
        if service:
            ...
            self.hass.async_create_task(
                self.hass.services.async_call(
                    domain,
                    name,
                    {"title": f"Hydroponic Control: {severity}", "message": message},
                    blocking=False,
                )
            )
