"""Shared entity helpers for Hydroponic Control."""
...
def hydro_device_info(entry) -> DeviceInfo:
    """Return the device that groups all Hydroponic Control entities."""
    return DeviceInfo(
...
class HydroEntity(CoordinatorEntity[HydroController]):
    """Base for read-only entities driven by the controller's data."""

    _attr_has_entity_name = True

    def __init__(self, controller: HydroController, key: str) -> None:
        """Initialise a coordinator-backed entity."""
        super().__init__(controller)
        self.controller = controller
        self._attr_unique_id = f"{controller.entry.entry_id}_{key}"
        self._attr_translation_key = key
        self._attr_device_info = hydro_device_info(controller.entry)


class HydroSettingEntity(Entity):
